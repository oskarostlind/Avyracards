      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-3 md:py-4">
            ...
            {isAuthenticated ? (
              <nav className="space-y-2">
                ...
                <Link
                  href="/dashboard"
                  onClick={closeMenu}
                  className="block rounded-lg px-3 py-2 text-sm text-nordic-secondary hover:bg-nordic-primary/70"
                >
                  Dashboard
                </Link>
              ...
              <nav className="space-y-3">
                ...
                <Link
                  href="/get-started"
                  onClick={closeMenu}
                  className="block w-full text-center rounded-lg bg-nordic-accent/10 border border-nordic-accent/40 px-3 py-2 text-sm font-medium text-nordic-accent hover:bg-nordic-accent/15"
                >
                  Kom igång
                </Link>
                <div className="px-3">
        ...
        <div className="hidden items-center justify-between md:flex">
          <div className="flex items-center gap-6">
            ...
            <nav
              aria-label="Segment"
              className="rounded-full border border-nordic-highlight/30 bg-nordic-primary/80 px-1 py-0.5 text-xs text-nordic-highlight shadow-sm"
            >
                ...
                <li>
                  <Link
                    ...
                    className={`rounded-full px-3 py-1 text-[11px] font-medium transition ${
                      inSocial
                        ? "bg-nordic-primary/70 text-nordic-secondary shadow-sm"
                        : "text-nordic-highlight hover:bg-nordic-primary/70 hover:text-nordic-secondary"
                    }`}
                  >
                    Socialt
                  </Link>
            ...
            {isAuthenticated ? (
              <>
                {isAdmin && (
                  <Link
                    href="/admin"
                    className="flex items-center gap-1.5 rounded-full bg-nordic-accent/10 px-3 py-1 text-xs font-bold text-nordic-accent border border-nordic-accent/40 hover:bg-nordic-accent/15 transition-colors"
                  >
                ...
                <Link
                  href="/dashboard"
                  className="text-nordic-highlight hover:text-nordic-accent transition-colors"
                >
                  Dashboard
                </Link>
              ...
              <>
                <Link
                  href="/get-started"
                  className="rounded-full border border-nordic-highlight/40 px-4 py-1.5 text-sm font-medium text-nordic-secondary hover:border-nordic-accent hover:text-nordic-accent transition-colors"
                >
                  Kom igång
                </Link>
                <SignInButton />
