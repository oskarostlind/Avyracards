...
export function ProfilePreviewModal({ isOpen, onClose, username }: ProfilePreviewModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-nordic-primary/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-[400px] h-[85vh] flex flex-col bg-nordic-primary rounded-[3rem] border-8 border-nordic-highlight/40 shadow-2xl overflow-hidden">
        ...
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 z-50 p-2 bg-nordic-primary/50 text-nordic-secondary rounded-full hover:bg-nordic-primary/70 backdrop-blur-md"
        >
          <X size={20} />
        </button>

        {/* Iframe Content */}
        <div className="flex-1 w-full h-full bg-nordic-primary pt-6">
            <iframe 
                src={url} 
      ...
      <div className="absolute top-4 right-4 text-nordic-secondary/50 text-sm hidden sm:block">
